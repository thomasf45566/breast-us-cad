...
appended "Errata (2026-09-07)") · **resolved-by-P3** (added 2026-09-07:
closed by the P3 commit — v2 artifacts published under v2/ on HF with
models/CHECKSUMS.txt; official BUS-BRA fold file committed as
data/splits/busbra_official_5fold.csv and sha256-verified at load).
...
| 8.9 | README:55–56 "Every number traces to a commit; the deployed demo shares the exact inference module" | as quoted | "Every number in RESULTS.md recomputes from committed CSV/JSON artifacts; the deployed demo shares the same model, calibration and inference module as the validation pipeline, while its preprocessing differs from the validation path on non-BUS-BRA image sizes (documented; impact on calibrated probability ≤ 7×10⁻³)"; README "Not in the repository" paragraph states v2 checkpoints are published nowhere; report §2.8 adds that external-v1 (2cbe1da) predates inference.py (2859043) | RESULTS.md:377–385, :1079–1091; RESULTS.md:372–375 (HF repo contents = v1 only at the time); git log (commit dates); **P3:** models/CHECKSUMS.txt | corrected; v2-weights part **resolved-by-P3** |
| 8.10 | README:63–64 repository map "data/splits/ official BUS-BRA folds", "models/ checkpoints" | as quoted | "data/splits/ frozen external keep-lists (busi/gdph/sysucc_clean.csv) — the BUS-BRA fold file is NOT here"; "models/ calibration + operating-point JSONs only; the .pt checkpoints are git-ignored"; new paragraph says the folds are the dataset's own `5-fold-cv.csv` in git-ignored `data/raw/busbra/`, v1 weights on HF, v2 weights unpublished. Same fix in CLAUDE.md rule 1. | .gitignore (`data/*`, `models/*.pt`); `git ls-files data/splits models` (3 CSVs; JSONs only); external_protocol.md:39, :102–103 (keep-list files); RESULTS.md:373–375 | corrected (README, CLAUDE.md); **resolved-by-P3**: fold file committed (data/splits/busbra_official_5fold.csv, CC BY 4.0 with notice), data.resolve_fold_file hash-verifies it, data/README.md:17 corrected, CLAUDE.md rule 1 and README map state the new location |
...
| 9.7 | v2 results | nine v2 checkpoints + BiomedCLIP export exist only on the author's disk | README and report §2.9 state this explicitly and that Q1/Q2 are reproducible only from committed prediction CSVs | RESULTS.md:373–375 (HF held v1 only); reports/v2_biomedclip_external_*_preds.csv, v2_loco_*_preds.csv; **P3:** models/CHECKSUMS.txt (sha256 of all 9 v2 ckpts, 10 JSONs, BiomedCLIP export, and the 8 v1 files), HF repo v2/ | corrected (disclosure); **resolved-by-P3** (published; inference.resolve_weight maps v2 names to v2/) |
...
| E.3 | §1 fold file location (CLAUDE.md:15, data/README.md:17, README:63) | CLAUDE.md and README corrected in P1. **P3:** licence check (Zenodo 8231412 = CC BY 4.0; bundled LICENSE.txt permits copying/distribution with the notice and citation) → official 5-fold-cv.csv committed verbatim as data/splits/busbra_official_5fold.csv (sha256 8bed2d2e…) with data/splits/BUSBRA_LICENSE.txt; data.resolve_fold_file prefers it, falls back to data/raw/busbra/5-fold-cv.csv, hash-verifies either; tests added; data/README.md:17 corrected | **resolved-by-P3** |
| E.4 | §3 self-certified pre-registration timeline; history rewrite; private repo; HF postdates external-v1 | New section "Provenance and limitations of pre-registration" in README and report §2.10 (P1). **P3:** the audit's "v2 checkpoints published nowhere" and "fold file outside the repository" parts of finding 3 are closed (see 8.10, 9.7, E.3). External timestamping | corrected (disclosure); artifacts **resolved-by-P3**; **P4 (2026-09-07):** docs/PROVENANCE.md written (tag timeline, identity-rewrite evidence incl. the full pre→post commit mapping by tree hash, HF timestamps, internal-only pre-registration statement, forward OSF commitment); CITATION.cff + .zenodo.json + `git archive` of tag v1.0-audited prepared for Zenodo — DOI pending the author's upload; visibility unchanged by design |
...
## H. P3 — provenance part 1 (2026-09-07)

| # | Audit item | Handling | Source | Status |
|---|---|---|---|---|
| H.1 | §9 "nine v2 checkpoints … exist only on the author's disk — not on HF, not in git" | scripts/upload_v2_weights.py: one HF commit adding v2/v2_biomedclip_fold{1-5}.pt, v2/v2_loco_{breast,busi,gdph,sysucc}.pt, their 10 calibration/operating-point JSONs and v2/pretrained/biomedclip_vitb16_timm.pt to happytommy/breast-us-cad-weights; models/CHECKSUMS.txt (sha256sum format, v1 + v2, 28 files) committed and uploaded; model card gains a "v2/ — research artifacts, not the deployed model" section; inference.hf_repo_path/resolve_weight resolve v2 names to v2/ (tests/test_hf_paths.py) | models/CHECKSUMS.txt; HF commit URL in the P3 commit message | resolved-by-P3 |
| H.2 | §1/§9 BUS-BRA fold file not in the repository (CLAUDE.md:15, data/README.md:17, README:63; `.gitignore data/*`) | Licence: Zenodo 8231412 → cc-by-4.0; bundled LICENSE.txt grants copy/distribute rights subject to the notice + citation → redistribution permitted. Committed verbatim as data/splits/busbra_official_5fold.csv (sha256 8bed2d2e…, byte-identical to data/raw/busbra/5-fold-cv.csv) with data/splits/BUSBRA_LICENSE.txt; src/data.py resolve_fold_file prefers it, falls back to the raw location, and refuses any file whose sha256 differs; tests/test_splits.py gains two tests; CLAUDE.md rule 1, README map, data/README.md corrected | data/splits/busbra_official_5fold.csv; data/splits/BUSBRA_LICENSE.txt | resolved-by-P3 |
| H.3 | §6/§9 self-check and tests after the change | `python src/external_val.py --self-check` output byte-identical to reports/external_selfcheck.txt; pytest green (see P3 commit message for counts) | reports/external_selfcheck.txt | resolved-by-P3 |
| H.4 | §10 `scripts/deploy_hf.py` rewrites src/inference.py; §6 warm-latency single value | Not changed in P3. Added to plan.md P6: (a) retry loop in verify_space.py's main comparison loop, (b) report Space warm latency as a 6–9 s range over three measurements | plan.md P6 | deferred-to-P6 |

## I. P4 — provenance part 2 (2026-09-07)

| # | Audit item | Handling | Source | Status |
|---|---|---|---|---|
| I.1 | §3 "no third-party record establishes that the freeze preceded the external scoring" | Cannot be created retroactively; stated as such in docs/PROVENANCE.md §4. Forward commitment (§6): every future protocol (MAMA-MIA, NTUH) is registered on OSF before any computation. Zenodo deposition of the audited repository gives the FIRST external timestamp for the whole trail (post hoc for external-v1, contemporaneous for the audit response) | docs/PROVENANCE.md | resolved-by-P4 (disclosure + forward commitment); the retroactive gap is permanent and stated |
| I.2 | §3 history rewrite: "pre-rewrite hashes cannot be mapped to any current commit except by content and timestamp" | The pre-rewrite commit objects still exist (unreachable) in the author's working copy; docs/PROVENANCE.md §3 tabulates every pre-rewrite commit → current commit with identical tree hash and author date, including the six hashes the audit found in wandb-metadata.json | docs/PROVENANCE.md §3 (table generated from `git fsck --unreachable` + tree match) | resolved-by-P4 (evidence documented; mapping reproducible only from this working copy, stated) |
| I.3 | §3 external anchors: HF weights 2026-08-30T04:25:49Z postdate external-v1 | HF creation and commit timestamps recorded verbatim in docs/PROVENANCE.md §5 with the ordering stated plainly | HF API (model_info / space_info / list_repo_commits) | resolved-by-P4 (recorded) |
| I.4 | §3 repository private at audit time | Public as of 2026-09-07 (https://github.com/thomasf45566/breast-us-cad), pushed with tags after the Zenodo deposition 10.5281/zenodo.22630912 (PROVENANCE.md §1, §7) | PROVENANCE.md | resolved (2026-09-07) |

## J. P4b — licensing and redistribution check (2026-09-07)

Scope: every tracked file under data/, reports/, app/examples/,
deploy/examples/ and docs/assets/ (`git ls-files`), checked for (a) images
not from BUS-BRA and (b) per-image reproduction of GDPH&SYSUCC's
`BIRADS&FOLD.xlsx` reader columns or of BrEaST's annotation table.

| # | Finding | Files | Assessment | Status |
|---|---|---|---|---|
| J.1 | (a) Figures embedding raw GDPH/SYSUCC images (dataset without explicit licence) | `reports/gradcam_external_fp.png` (8 benign images, 4 SYSUCC + 4 GDPH, with CAM overlays); `reports/phash_cross_pairs.png` (3 GDPH/SYSUCC images + 1 BUS-BRA); `reports/phash_within_d8_sample.png` (8 SYSUCC images) | Redistribution rights not established. **Ruling (P4c): excluded from distribution** — the three PNGs untracked (`git rm --cached`), listed in `.gitignore`, kept locally. Replacements from saved artifacts: `reports/gradcam_external_fp_breast.png` (same design, 8 highest-p BrEaST benign FPs, CC BY 4.0; src/explain_breast_fp.py), `reports/phash_cross_pairs_table.csv` + `reports/phash_cross_pairs_busbra_thumb.png` (BUS-BRA side only), `reports/phash_within_d8_table.csv` (src/phash_tables.py). RESULTS.md Errata entry appended; report appendix A and §2.4/§3.6 updated. | **resolved-by-P4c** |
| J.2 | (a) Figure embedding raw Curated BUSI images | `reports/busi_dup_pairs.png` (14 images, the 7 dropped near-duplicate pairs) | CC BY 4.0: permitted with the citation in THIRD_PARTY_DATA.md §3 | corrected (attribution recorded) |
| J.3 | (a) BUS-BRA images in examples, README screenshot and internal figures | `app/examples/*.png`, `deploy/examples/*.png`, `docs/assets/demo.png`, 10 internal figures listed in THIRD_PARTY_DATA.md §1 | CC BY 4.0 + bundled permission notice: permitted; notice committed as `data/splits/BUSBRA_LICENSE.txt`, pointed to from README and deploy/README | corrected |
| J.4 | (b) Per-image reader BI-RADS columns from `BIRADS&FOLD.xlsx` | none — all CSV/JSON headers inspected; keyword hits were the word "fold"; `posthoc_reader_concordance.csv` and `birads_comparison_*.png` are aggregate only | nothing to replace | corrected (verified absent) |
| J.5 | (b) BrEaST annotation table | only the binary `Classification` label per case (as `y_true`) in the prediction/member CSVs; no BI-RADS, shape, margin or other columns | CC BY 4.0: permitted with citation | corrected (verified; documented) |
| J.6 | GDPH/SYSUCC filenames + filename-encoded class in keep-lists and prediction CSVs | `data/splits/{gdph,sysucc}_clean.csv`, `reports/external_{gdph,sysucc}_preds.csv`, v2 preds/members, `phash_sweep_hits.csv`, `v2_resize_dispatch_impact.csv` | No pixels, no xlsx annotation. **Ruling (P4c): identifiers retained for reproducibility** (THIRD_PARTY_DATA.md §4) | resolved-by-P4c |
| J.7 | Licensing | `LICENSE` (Apache-2.0, canonical text) added; README "Licences" section (code Apache-2.0; docs/ and authored figures CC BY 4.0; examples → BUSBRA_LICENSE.txt); `THIRD_PARTY_DATA.md` with per-dataset licence, verbatim citation and derived-file inventory; CITATION.cff `license: Apache-2.0`; .zenodo.json `license: apache-2.0`; PROVENANCE.md §2 rows reordered chronologically | — | corrected |
