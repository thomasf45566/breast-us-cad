...
|---|---|---|---|---|
| J.1 | (a) Figures embedding raw GDPH/SYSUCC images (dataset without explicit licence) | `reports/gradcam_external_fp.png` (8 benign images, 4 SYSUCC + 4 GDPH, with CAM overlays); `reports/phash_cross_pairs.png` (3 GDPH/SYSUCC images + 1 BUS-BRA); `reports/phash_within_d8_sample.png` (8 SYSUCC images) | Redistribution rights not established. Proposed action if the author rules against redistribution: replace each with an aggregate-only artifact (CAM heat statistics / pair-distance table without pixels), untrack the PNGs, keep them locally, and note the replacement in RESULTS.md's Errata. | **pending author decision — nothing untracked yet** |
