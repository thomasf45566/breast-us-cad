...
| 2.2 | "sensitivity ≥ 0.92 on all four" (RESULTS.md:485, :1058; plan.md:54; app/app.py:67; deploy/README.md:31) | "sensitivity held ≥ 0.92 at the frozen threshold" | "sensitivity held ≥ 0.918 at the frozen threshold" in plan.md, app/app.py, deploy/app.py, deploy/README.md; report §3.11/§4.1/§4.2 "全域 ≥ 0.92" → "≥ 0.918". BrEaST sens = 90/98 = 0.9184. | RESULTS.md:185 (0.9184), :201 (90/91/8/63) | corrected (4 doc places); **deferred-to-P2** for RESULTS.md:485 and :1058 — RESULTS.md may not be edited in P1; erratum recorded here and to be carried into the P2 POST-HOC section |
...
| 8.21 | Report §3.1:84 "121 張惡性僅 3 張 …" | as quoted | see A-2.6a | see A-2.6a | deleted |
...
| 8.26 | Report §4.2:148 "模型與判讀者被同一批非典型良性以同一方式誤導"; "SYSUCC 之 disagreement 反轉為第三方印證" | as quoted | "*未檢定之詮釋:外部 FP 之 Grad-CAM 熱區落於病灶本體,與「非典型良性外觀」相符;本研究未做逐影像之模型—判讀者一致性分析,故「模型與判讀者被同一批影像誤導」為假說而非結果。*" The "第三方印證" sentence is removed. | RESULTS.md:300–310 (external FP CAMs, "do not quantify") | corrected (restated as untested interpretation) / deleted (第三方印證) |
...
|---|---|---|---|
| E.1 | §1 epoch-selection bias (best-epoch-on-the-same-fold; pooled OOF inherits it) | Disclosed in README (headline note, Limitations), report (§2.2, abstract, §3.1, §4.5), plan.md (cv_vit entry). Source: reports/cv_vit_summary.csv `best_epoch`. Quantification (epoch-selection sensitivity, nested out-of-sample operating point) | corrected (disclosure); **deferred-to-P2** (quantification) |
| E.2 | §1 misleading checkpoint metadata (`train_folds=[1,2,3,4]` stored in every CV ckpt) | Code path (`train.py`) — not touched in P1 | **deferred-to-P2** (plan.md P2: "train.py stores resolved folds") |
...
| E.7 | §4 Q1 wording exceeds the fired branch | See B-7.5; the RESULTS.md:1028–1033 sentence itself is untouched in P1 | corrected (docs); **deferred-to-P2** (RESULTS.md wording, alongside the ≥ 0.92 and 1,879 errata) |
...
## F. RESULTS.md errata carried forward (not editable in P1)

These sentences in `RESULTS.md` are wrong or imprecise per the audit and
are corrected in every other document; they are listed here so the P2
session can add them to the POST-HOC/errata section:
